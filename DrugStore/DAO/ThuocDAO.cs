...
namespace DAO
{
    public class ThuocDAO
    {
        ...
        {
            return DataProvider.Instance.ExecuteQuery("SELECT SODK, TENTHUOC, HOATCHAT, GIANHAP FROM Thuoc JOIN NhaCungCap ON Thuoc.MANCC = NhaCungCap.MANCC WHERE TENNCC = @tenncc", new object[] { tenncc });
        }
        ...
        {
            String query = "select t.SODK, t.TENTHUOC, kt.MAPHIEU, DATEDIFF(DAY,GETDATE(),kt.HSD) AS HSD, kt.SOLUONG from Thuoc t join KhoThuoc kt ON t.SODK = kt.SODK WHERE DATEDIFF(month,GETDATE(),kt.HSD) BETWEEN @hsd1 AND @hsd2";
            return DataProvider.Instance.ExecuteQuery(query, new object[] { hsd1, hsd2 });
        }
        public DataTable timThuocTrongKho(String tenthuoc)
        {
            String query = String.Format("select t.SODK, t.TENTHUOC, kt.MAPHIEU, DATEDIFF(DAY,GETDATE(),kt.HSD) AS HSD, kt.SOLUONG from Thuoc t join KhoThuoc kt ON t.SODK = kt.SODK WHERE DATEDIFF(month,GETDATE(),kt.HSD) BETWEEN 0 AND {0} AND t.TENTHUOC LIKE N'{1}%'", Int32.MaxValue,tenthuoc);
            return DataProvider.Instance.ExecuteQuery(query);
        }
