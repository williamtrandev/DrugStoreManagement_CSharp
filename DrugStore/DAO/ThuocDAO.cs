...
namespace DAO
{
    public class ThuocDAO
    {
        ...
        {
            String query = String.Format("SELECT Thuoc.SODK, Thuoc.TENTHUOC, MAPHIEU, HSD, SOLUONG FROM KhoThuoc JOIN Thuoc ON Thuoc.SODK = KhoThuoc.SODK WHERE (DATEDIFF(MONTH, GETDATE(), HSD) BETWEEN 0 AND {0}) AND Thuoc.TENTHUOC LIKE N'{1}%'", Int32.MaxValue, tenthuoc);
            return DataProvider.Instance.ExecuteQuery(query);
        }
